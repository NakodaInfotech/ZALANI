Imports BL
Public Class JobOrderForLaminationDetails

    Dim USERADD, USEREDIT, USERVIEW, USERDELETE As Boolean      'USED FOR RIGHT MANAGEMAENT
    Public TEMPJONO As String
    Private Sub JobOrderForLaminationDetails_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub JobOrderForLaminationDetails_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
    ...
    Sub FILLGRID()
        Try
            Dim OBJDEPT As New ClsJobOrderForLamination
    ...
    Sub showform(ByVal editval As Boolean, ByVal TEMPJONO As Integer)
        Try
            ...
            If (editval = False) Or (editval = True And gridbill.RowCount > 0) Then
                Dim objgdn As New JobOrderForLamination
    ...
    Private Sub TOOLPRINT_Click(sender As Object, e As EventArgs) Handles TOOLPRINT.Click
        Try
            ...
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                ...
                If MsgBox("Wish to Print Job Order for Lamination from " & TXTFROM.Text.Trim & " To " & TXTTO.Text.Trim & " ?", MsgBoxStyle.YesNo) = vbYes Then
                    SERVERPROPDIRECT()
                End If
            Else
                If MsgBox("Wish to Print Selected Job Order for Lamination ?", MsgBoxStyle.YesNo) = vbYes Then
                    SERVERPROPSELECTED()
                End If
            End If
        Catch ex As Exception
    ...
    Sub SERVERPROPDIRECT(Optional ByVal INVOICEMAIL As Boolean = False, Optional ByVal WHATSAPP As Boolean = False)
        Try
            ...
            For I As Integer = Val(TXTFROM.Text.Trim) To Val(TXTTO.Text.Trim)
                ...
                ALATTACHMENT.Add(Application.StartupPath & "\JOLAMINATIONREPORT_" & I & ".pdf")
                FILENAME.Add("JOLAMINATIONREPORT_" & I & ".pdf")
            Next

            If INVOICEMAIL Then
                Dim OBJMAIL As New SendMail
                OBJMAIL.ALATTACHMENT = ALATTACHMENT
                OBJMAIL.subject = "JOLAMINATIONREPORT"
                OBJMAIL.ShowDialog()
            End If
    ...
    Sub SERVERPROPSELECTED(Optional ByVal INVOICEMAIL As Boolean = False, Optional ByVal WHATSAPP As Boolean = False)
        Try
            ...
            For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                ...
                OBJPO.Show()
                OBJPO.Close()
                ALATTACHMENT.Add(Application.StartupPath & "\JOLAMINATIONREPORT_" & Val(ROW("TEMPJONO")) & ".pdf")
                FILENAME.Add("JOLAMINATIONREPORT_" & Val(ROW("TEMPJONO")) & ".pdf")
            Next
