Imports BL
Public Class JobOrderForSlittingDetails
    ...
    Private Sub JobOrderForSlittingDetails_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub JobOrderForSlittingDetails_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
    ...
    Sub FILLGRID()
        Try
            Dim OBJDEPT As New ClsJobOrderForSlitting
            OBJDEPT.alParaval.Add(0)
            OBJDEPT.alParaval.Add(CmpId)
            OBJDEPT.alParaval.Add(YearId)
            Dim DT As DataTable = OBJDEPT.selectJOSLITTING(0, CmpId, YearId)
    ...
    Sub showform(ByVal editval As Boolean, ByVal TEMPJONO As Integer)
        Try
            ...
            If (editval = False) Or (editval = True And gridbill.RowCount > 0) Then
                Dim objgdn As New JobOrderForSlitting
    ...
    Private Sub TOOLPRINT_Click(sender As Object, e As EventArgs) Handles TOOLPRINT.Click
        Try
            ...
            If Val(TXTFROM.Text.Trim) > 0 And Val(TXTTO.Text.Trim) > 0 Then
                ...
                If MsgBox("Wish to Print Job Order for Conversion from " & TXTFROM.Text.Trim & " To " & TXTTO.Text.Trim & " ?", MsgBoxStyle.YesNo) = vbYes Then
                    SERVERPROPDIRECT()
                End If
            Else
                If MsgBox("Wish to Print Selected Job Order for Conversion ?", MsgBoxStyle.YesNo) = vbYes Then
                    SERVERPROPSELECTED()
                End If
            End If
        Catch ex As Exception
    ...
    Sub PRINTREPORT(ByVal TEMPJONO As Integer)
        Try
    ...
    Sub SERVERPROPSELECTED(Optional ByVal INVOICEMAIL As Boolean = False, Optional ByVal WHATSAPP As Boolean = False)
        Try
            ...
            For I As Integer = 0 To Val(SELECTEDROWS.Length - 1)
                ...
                ALATTACHMENT.Add(Application.StartupPath & "\JOSLITTINGREPORT_" & Val(ROW("TEMPJONO")) & ".pdf")
                FILENAME.Add("JOSLITTINGREPORT_" & Val(ROW("TEMPJONO")) & ".pdf")
            Next
    ...
    Private Sub TOOLEXCEL_Click(sender As Object, e As EventArgs) Handles TOOLEXCEL.Click
        Try
            ...
            Dim PATH As String = Application.StartupPath & "\ JOB ORDER FOR SLITTING Details.XLS"
            Dim opti As New DevExpress.XtraPrinting.XlsExportOptions
            opti.ShowGridLines = True
            opti.SheetName = " JOB ORDER FOR SLITTING Details"
            gridbill.ExportToXls(PATH, opti)
            EXCELCMPHEADER(PATH, " JOB ORDER FOR SLITTING Details", gridbill.VisibleColumns.Count + gridbill.GroupCount)
        Catch ex As Exception
            MsgBox(" JOB ORDER FOR SLITTING Details Excel File is Open, Please Close the File first then try to Export", MsgBoxStyle.Critical)
        End Try
    End Sub
