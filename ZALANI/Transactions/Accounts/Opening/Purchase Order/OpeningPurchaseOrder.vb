...
Public Class OpeningPurchaseOrder
    ...
    Private Sub OpeningPurchaseOrder_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
...
LINE1:
    ...
    Private Sub txtpono_Validating(sender As Object, e As CancelEventArgs) Handles TXTPONO.Validating
        Try
            If Val(TXTPONO.Text.Trim) <> 0 And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim DTTABLE As DataTable = OBJCMN.SEARCH("ISNULL(OPENINGPURCHASEORDER.OPO_NO,0)  AS PONO", "", " OPENINGPURCHASEORDER ", "  AND OPENINGPURCHASEORDER.OPO_NO=" & Val(TXTPONO.Text.Trim) & " AND OPENINGPURCHASEORDER.OPO_YEARID = " & YearId)
                If DTTABLE.Rows.Count > 0 Then
                    MsgBox("Opening  PURCHASE Order No Already Exist")
                    e.Cancel = True
                End If
            End If
        Catch ex As Exception
    ...
    Private Sub OpenToolStripButton_Click(sender As Object, e As EventArgs) Handles OpenToolStripButton.Click
        Try
            If USEREDIT = False And USERVIEW = False Then
                ...
                Exit Sub
            End If
            Dim OBJPAYDTLS As New OpeningPurchaseOrderDetails
    ...
    Private Sub gridOPO_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles GRIDPO.CellDoubleClick
        EDITROW()
    End Sub

    Private Sub gridOPO_KeyDown(sender As Object, e As KeyEventArgs) Handles GRIDPO.KeyDown
        Try
    ...
    Private Sub OpeningPurchaseOrder_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
    ...
    Sub GETPURCHASE(STORESITEMNAME As String)
        Try
            GRIDPUR.RowCount = 0
            Dim OBJCMN As New ClsCommon
            Dim DT As DataTable = OBJCMN.SEARCH(" TOP 5 ISNULL(OPENINGPURCHASEORDER.OPO_NO,0) AS BILLNO, OPENINGPURCHASEORDER.OPO_DATE AS BILLDATE, ISNULL(LEDGERS.ACC_CMPNAME,'') AS NAME, ISNULL(OPENINGPURCHASEORDER_DESC.OPO_QTY,0) AS QTY, ISNULL(OPENINGPURCHASEORDER_DESC.OPO_RATE,0) AS RATE, ISNULL(OPENINGPURCHASEORDER_DESC.OPO_AMT,0) AS AMT  ", "", " OPENINGPURCHASEORDER INNER JOIN OPENINGPURCHASEORDER_DESC ON OPENINGPURCHASEORDER.OPO_NO = OPENINGPURCHASEORDER_DESC.OPO_NO AND OPENINGPURCHASEORDER.OPO_YEARID = OPENINGPURCHASEORDER_DESC.OPO_YEARID INNER JOIN LEDGERS ON OPENINGPURCHASEORDER.OPO_LEDGERID = ACC_ID INNER JOIN ITEMMASTER ON OPO_QUALITYID  = ITEM_ID ", " AND ITEMMASTER.ITEM_NAME = '" & STORESITEMNAME & "' ORDER BY OPENINGPURCHASEORDER.OPO_DATE DESC  ")
    ...
    Private Sub TXTSIZE_Validated(sender As Object, e As EventArgs) Handles TXTSIZE.Validated
        Try
            If TXTSIZE.Text.Trim <> "" And TXTGSM.Text.Trim <> "" And CMBQUALITY.Text <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.SEARCH("ISNULL(ITEMNAME, '') AS QUALITY, ISNULL(GSM, '') AS GSM, ISNULL(SIZE, '') AS SIZE, ISNULL(QTY, 0) AS QTY", "", "STOCKREGISTER", " AND STOCKREGISTER.ITEMNAME='" & CMBQUALITY.Text.Trim & "'AND STOCKREGISTER.GSM='" & TXTGSM.Text.Trim & "' AND STOCKREGISTER.SIZE=" & TXTSIZE.Text.Trim)
                If DT.Rows.Count > 0 Then
                    TXTSTOCK.Text = DT.Rows(0).Item("QTY")
                    TOTAL()
                End If
            End If
        Catch ex As Exception
