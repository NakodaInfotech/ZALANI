...
Imports BL
Public Class OpeningProformaInvoice
    ...
    Sub getmax_PRONO()
        ...
        DTTABLE = getmax(" isnull(max(INVOICE_NO),0) + 1 ", "OPENINGPROFORMAINVOICEMASTER", "  AND INVOICE_yearid=" & YearId)
        If DTTABLE.Rows.Count > 0 Then TXTPRONO.Text = DTTABLE.Rows(0).Item(0)
    End Sub

    Private Sub OpeningProformaInvoice_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub OpeningProformaInvoice_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            ...
            If EDIT = True Then
                ...
                Dim OBJCLSPROFORMA As New ClsOpeningProformaInvoice
                ...
                If dttable.Rows.Count > 0 Then
                    ...
                    Dim dtable As DataTable = OBJCMN.SEARCH(" OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_ESRNO AS GRIDSRNO, LEDGERS.Acc_cmpname AS CHARGES, OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_PER AS PER, OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_AMT AS AMT, OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_TAXID AS TAXID ", "", " OPENINGPROFORMAINVOICEMASTER_CHGS INNER JOIN LEDGERS ON OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_CHARGESID = LEDGERS.Acc_id  ", " AND OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_NO = " & TEMPPORFORMA & " AND OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_YEARID = " & YearId & " ORDER BY OPENINGPROFORMAINVOICEMASTER_CHGS.INVOICE_Esrno")
    ...
    Private Sub cmdok_Click(sender As Object, e As EventArgs) Handles CMDOK.Click
        Try
            ...
            Dim objclsPurord As New ClsOpeningProformaInvoice()
    ...
    Sub PRINTREPORT(ByVal PROFORMANO As Integer)
        Try
            If MsgBox("Wish to Print Opening Proforma Invoice?", MsgBoxStyle.YesNo) = vbNo Then Exit Sub
    ...
    Private Function ERRORVALID() As Boolean
        ...
        If ALLOWMANUALINVNO = True Then
            If Val(TXTPRONO.Text.Trim) <> 0 And CMBNAME.Text.Trim <> "" And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.SEARCH(" ISNULL(OPENINGPROFORMAINVOICEMASTER.INVOICE_NO, '') AS INVOICENO ", "", " OPENINGPROFORMAINVOICEMASTER ", "  AND INVOICE_NO = " & Val(TXTPRONO.Text.Trim) & " AND OPENINGPROFORMAINVOICEMASTER.INVOICE_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    EP.SetError(TXTPRONO, "Opening Proforma No Already Exist")
                    bln = False
                End If
            End If
        End If
    ...
    Private Sub cmddelete_Click(sender As Object, e As EventArgs) Handles CMDDELETE.Click
        Try
            ...
            If EDIT = True Then
                ...
                If MsgBox("Delete Opening Proforma Invoice?", MsgBoxStyle.YesNo) = vbYes Then
                    Dim alParaval As New ArrayList
                    alParaval.Add(Val(TEMPPORFORMA))
                    alParaval.Add(CmpId)
                    alParaval.Add(YearId)

                    Dim ClsINCTAG As New ClsOpeningProformaInvoice()
                    ClsINCTAG.alParaval = alParaval
                    Dim IntResult As Integer = ClsINCTAG.DELETE()
                    MsgBox("Opening Proforma Invoice Deleted")
                    CLEAR()
                    EDIT = False
...
LINE1:
            ...
            If TEMPPORFORMA > 0 Then
                EDIT = True
                OpeningProformaInvoice_Load(sender, e)
            Else
                CLEAR()
                EDIT = False
            End If
...
LINE1:
    ...
    Private Sub OpenToolStripButton_Click(sender As Object, e As EventArgs) Handles OpenToolStripButton.Click
        Try
            ...
            Dim objINVDTLS As New OpeningProformaInvoiceDetails
    ...
    Private Sub GRIDPROFORMA_KeyDown(sender As Object, e As KeyEventArgs) Handles GRIDPROFORMA.KeyDown
        Try
            ...
            ElseIf e.KeyCode = Keys.F5 Then
                EDITROW()
            ElseIf e.KeyCode = Keys.F12 And GRIDPROFORMA.RowCount > 0 Then
                If GRIDPROFORMA.CurrentRow.Cells(GFINISHEDQUALITY.Index).Value <> "" Then GRIDPROFORMA.Rows.Add(CloneWithValues(GRIDPROFORMA.CurrentRow))
    ...
    Sub Fillgrid()
        Try
            If GRIDDOUBLECLICK = False Then
                GRIDPROFORMA.Rows.Add(Val(TXTSRNO.Text.Trim), CMBFINISHED.Text.Trim, TXTDESC.Text.Trim, TXTGSM.Text.Trim, TXTGSMDETAILS.Text.Trim, TXTSIZE.Text.Trim, CMBFOIL.Text.Trim, TXTFOILGSM.Text.Trim, TXTPEGSM.Text.Trim, TXTROLLDIAMETER.Text.Trim, TXTCOREPIPEWIDTH.Text.Trim, CMBPALLETIZED.Text.Trim, TXTNARRATION.Text.Trim, Format(Val(TXTQTY.Text.Trim), "0.00"), CMBUNIT.Text.Trim, Format(Val(TXTRATE.Text.Trim), "0.00"), Format(Val(TXTAMOUNT.Text.Trim), "0.00"), 0, 0, 0, "OPENING")
                getsrno(GRIDPROFORMA)
    ...
    Async Sub SENDWHATSAPP(INVNO As Integer)
        Try
            ...
            OBJSO.FORMULA = "{OPENINGPROFORMAINVOICEMASTER.INVOICE_NO}=" & Val(INVNO) & " and {OPENINGPROFORMAINVOICEMASTER.INVOICE_YEARID}=" & YearId
            OBJSO.INVNO = INVNO
            OBJSO.NOOFCOPIES = 1
            OBJSO.Show()
            OBJSO.Close()
